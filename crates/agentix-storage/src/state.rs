...
use agentix_domain::{
    AttachOutcome, ChannelKind, ConversationRef, MessageRef, SessionId, TurnStatus,
};

#[derive(Debug, Clone)]
pub struct StoredTurnView {
...
impl SqliteState {
    ...
    pub async fn uncertain_event_count(&self) -> Result<i64, sqlx::Error> {
        sqlx::query_scalar("SELECT COUNT(*) FROM processed_events WHERE status = 'uncertain'")
            .fetch_one(&self.pool)
            .await
    }

    /// Qualify legacy runtime records without rewriting upstream session IDs.
    pub async fn qualify_sessions(
        &self,
        legacy: Option<agentix_domain::AgentKind>,
    ) -> Result<(), sqlx::Error> {
        let mut tx = self.pool.begin().await?;
        for table in ["bindings", "pending_interactions", "turn_views"] {
            let ids: Vec<String> =
                sqlx::query_scalar(&format!("SELECT DISTINCT session_id FROM {table}"))
                    .fetch_all(&mut *tx)
                    .await?;
            for id in ids {
                if agentix_domain::SessionKey::decode(&SessionId::new(&id)).is_some() {
                    continue;
                }
                let kind = legacy.ok_or_else(|| sqlx::Error::Protocol("Legacy sessions need an owner: start once with the original [agent] configuration before enabling [[agents]]".into()))?;
                let qualified = agentix_domain::SessionKey::new(kind, SessionId::new(&id)).encode();
                sqlx::query(&format!(
    ...
    pub async fn binding_epoch(&self, conversation: &ConversationRef) -> Result<u64, sqlx::Error> {
        let epoch =
    ...
    /// Prevent automatic replay after an interrupted operation whose remote
    /// outcome cannot be established. Completed or explicitly failed requests
    /// retain their existing state; only an in-flight claim becomes uncertain.
    pub async fn fence_event(
        &self,
        channel: ChannelKind,
        event_id: &str,
    ) -> Result<(), sqlx::Error> {
        sqlx::query(
            "INSERT INTO processed_events (channel, event_id, status, attempts) \
             VALUES (?, ?, 'uncertain', 1) \
             ON CONFLICT(channel, event_id) DO UPDATE SET \
                status = 'uncertain', processed_at = unixepoch() \
    ...
    pub async fn save_turn_view(&self, view: &StoredTurnView) -> Result<(), sqlx::Error> {
        sqlx::query(
    ...
    pub async fn list_turn_views(&self) -> Result<Vec<StoredTurnView>, sqlx::Error> {
        let rows = sqlx::query(
