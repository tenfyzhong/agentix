...
async fn killed_cli_after_database_commit_replays_without_duplicates_and_repairs_files() {
    ...
    let store = agentix_task::Store::open(&cli.dir.path().join("state.sqlite3"))
        ...
        .unwrap();
    let lock =
        std::fs::File::open(cli.dir.path().join("vault/Tasks \u{2603}/.taskix.lock")).unwrap();
    lock.lock().unwrap();
    let args = [
        "task",
        "add",
...
#[test]
fn standalone_json_workflow_in_obsidian() {
    let cli = Cli::new();
    let project = cli.ok(&[
        "project",
        "register",
    ...
    assert!(context["documents"].get("format").is_none());
    cli.ok(&[
        "task",
        "done",
        tid,
...
fn invalid_arguments_and_business_errors_have_distinct_exit_codes() {
    ...
    let version = Command::new(env!("CARGO_BIN_EXE_taskix"))
        .arg("--version")
        .output()
        .unwrap();
    assert!(String::from_utf8_lossy(&version.stdout).starts_with("taskix "));
}
...
fn hooks_consume_host_json_and_do_not_require_a_job() {
    use std::io::Write;
    let cli = Cli::new();
    let mut child = Command::new(env!("CARGO_BIN_EXE_taskix"))
        .arg("--config")
...
#[test]
fn taskix_preserves_authored_language_without_language_configuration() {
    let cli = Cli::new();
    ...
    let dashboard =
        std::fs::read_to_string(cli.dir.path().join("vault/Tasks ☃/Dashboard.base")).unwrap();
    assert!(dashboard.contains("Projects"), "{dashboard}");
    assert!(
    ...
    let output = cli
        .command(&["sync"])
        .env("TASKIX_LANGUAGE", "unsupported")
        .output()
        .unwrap();
    assert!(output.status.success());
...
fn task_note_timestamps_follow_the_system_local_zone() {
    ...
    if let Ok(expected) = std::env::var("TASKIX_TEST_EXPECTED_OFFSET_SECONDS") {
