...
fn completions_skip_configuration_and_task_state_and_match_checked_in_files() {
    ...
    for (shell, file, registration) in [
        ("bash", "taskix.bash", "complete -F _taskix"),
        ("zsh", "_taskix", "#compdef taskix"),
        ("fish", "taskix.fish", "complete -c taskix"),
    ] {
...
fn bash_completes_nested_commands_options_and_paths() {
    ...
    let completion = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../completions/taskix.bash");
    for (words, index, expected) in [
        ("taskix ta", "1", "task"),
        ("taskix task cl", "2", "claim"),
        ("taskix task start --le", "3", "--lease-token"),
        ("taskix plan create --fi", "3", "--file"),
        ("taskix job list --ar", "3", "--archived"),
        ("taskix completions fi", "2", "fish"),
        ("taskix --config ex", "2", "example config.toml"),
        ("taskix plan create task_ID --file pl", "5", "plan draft.md"),
    ] {
...
COMP_CWORD=$3
_taskix "${COMP_WORDS[0]}" "${COMP_WORDS[COMP_CWORD]}" "${COMP_WORDS[COMP_CWORD-1]}"
printf '%s\n' "${COMPREPLY[@]}""#,
                "completion-test",
            ])
            .arg(&completion)
