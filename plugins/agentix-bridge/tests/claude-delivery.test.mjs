...
import { RmuxDelivery } from '../claude/delivery.mjs';
...
function fixture(overrides = {}) {
    ...
    return { calls, delivery: new RmuxDelivery({ env: { TMUX: '/tmp/rmux-501/default,1,0', TMUX_PANE: '%91' }, pid: 200, run, delay: async () => {} }) };
}
test('rmux pastes literal multiline text into the original pane before Enter', async () => {
    ...
    assert.ok(calls.filter(c => c.command === 'rmux').every(c => c.args[0] === '-S'));
});
for (const overrides of [ { foreground: '200 300' }, { state: '100|fish|2|1|0|0' }, { state: '999|claude|2|1|0|0' }, { state: '100|claude|2|1|1|0' }, { screen: 'Allow this tool?\n❯ Yes\n' } ]) {
    test(`rmux rejects an unsafe pane: ${JSON.stringify(overrides)}`, async () => {
...
test('rmux rejects missing pane context and terminal control input', async () => {
    await assert.rejects(new RmuxDelivery({ env: {} }).send({ text: 'hello' }));
...
test('rmux delivery is acknowledged by the matching prompt hook and completed by Stop', async () => {
    const sent = [];
    const session = new ClaudeSession({ delivery: { kind: 'rmux', send: async r => sent.push(r) }, ackTimeout: 100 });
...
test('rmux refuses unmatched hooks as delivery acknowledgements and preserves uncertainty', async () => {
    const session = new ClaudeSession({ delivery: { kind: 'rmux', send: async () => {} }, ackTimeout: 10 });
    session.hook(identity);
...
test('preflight failures leave no uncertain receipt and allow a later retry', async () => {
    let attempts = 0;
    const session = new ClaudeSession({ delivery: { kind: 'rmux', send: async () => {
...
test('Claude status identifies its prompt delivery adapter', async () => {
    const session = new ClaudeSession({ delivery: { kind: 'rmux', send: async () => {} } }); session.hook(identity);
    assert.match((await session.request('command', { name: 'status' })).body, /Delivery: rmux/);
});
test('native rmux clears a multiline draft and submits only the IM prompt', {
    skip: !process.env.AGENTIX_TEST_NATIVE_HOSTS || process.platform === 'win32', timeout: 15000,
}, async t => {
    const { execFileSync } = await import('node:child_process');
    const { mkdtempSync, readFileSync, rmSync, symlinkSync } = await import('node:fs');
    ...
    t.after(() => { try { execFileSync('rmux', ['-S', socket, 'kill-server']); } catch {} rmSync(root, { recursive: true, force: true }); });
    const quote = value => "'" + value.replaceAll("'", "'\\''") + "'";
    symlinkSync(process.execPath, join(root, 'claude'));
    const command = join(root, 'claude') + ' ' + [new URL('./claude-rmux-host.mjs', import.meta.url).pathname, output, 'local draft\nsecond line'].map(quote).join(' ');
    const pane = execFileSync('rmux', ['-S', socket, 'new-session', '-d', '-s', 'test', '-x', '100', '-y', '30', '-P', '-F', '#{pane_id}', command], { encoding: 'utf8' }).trim();
    await waitFor(() => execFileSync('rmux', ['-S', socket, 'capture-pane', '-p', '-t', pane], { encoding: 'utf8' }).includes('❯'), 5000);
    const pid = Number(execFileSync('rmux', ['-S', socket, 'display-message', '-p', '-t', pane, '#{pane_pid}'], { encoding: 'utf8' }).trim());
    const delivery = new RmuxDelivery({ env: { TMUX: `${socket},1,0`, TMUX_PANE: pane }, pid });
...
test('closing a session cancels outstanding delivery work', async () => {
    let signal;
    const session = new ClaudeSession({ delivery: { kind: 'rmux', send: async r => { signal = r.signal; } } }); session.hook(identity);
    const pending = session.request('prompt', { request_id: 'one', text: 'hello' });
    await new Promise(resolve => setImmediate(resolve));
    session.close();
    await assert.rejects(pending, e => e.bridgeCode === 'delivery_uncertain');
