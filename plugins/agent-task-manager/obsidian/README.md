...
After configuring taskcli with `init --root /existing/vault`, run `taskcli obsidian setup`. This installs TaskNotes and the embedded Taskcli Sync desktop plugin, enables both and Bases in the vault configuration, and merges the settings described below. When files change, setup uses Obsidian CLI to reload the configured vault. Enable the CLI in **Settings → General** and disable Restricted mode if needed. If the CLI is unavailable or reload fails, the installation is retained and setup asks you to restart Obsidian. Use `--no-reload` with Obsidian closed to skip CLI calls, then reopen it. Existing compatible installations are retained; `--plugin-dir /path/to/release` supports offline installation. Changed files are backed up under `.obsidian/taskcli-backups/`. See [automatic setup](https://github.com/tenfyzhong/agentix/blob/main/docs/task-board.md#obsidian-plugin-setup) for details.
