...
```sh
taskcli context --project prj_ID --session HOST_SESSION --json
# After the AI compares all candidates with the current request:
taskcli job create --project prj_ID --title "Login repair" --prompt "Fix login and cover it with tests" --inbox inbox_LOGIN --inbox inbox_TESTS --executor agent:HOST --session HOST_SESSION --json
# Associate additional semantic matches with an existing active Job:
taskcli job update job_ID --inbox inbox_ID --executor agent:HOST --session HOST_SESSION --json
```
