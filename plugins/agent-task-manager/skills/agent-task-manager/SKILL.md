...
5. While holding the lease, create or revise the Plan body in the Task note. Choose the content and structure freely to suit the task; no fixed headings or section template are required. Call `task start` with the same lease only after reviewing the current Plan and completing dependencies. Proceed with implementation only when start succeeds (`EXECUTING`). Refresh the Task revision after Plan writes.
