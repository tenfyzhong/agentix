...
- Publish Plan bodies through `plan create` / `plan revise`; do not directly overwrite registered Plan files. A Plan revision updates the same file, increments its version property, and requires ownership; do not create version directories or retain intermediate Plan files. After editing Notes, run `sync`. A projection warning means the database write succeeded; run `sync` to repair without creating duplicate work.
