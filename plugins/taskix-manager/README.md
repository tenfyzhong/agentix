...
Job creation captures the original user request with `job create --prompt`, separately from the summarized Goal. The generated Job note displays it as literal text under **Prompt**; sync, renaming, and archival preserve it. The skill keeps the prompt in its original language and wording. Existing active Jobs can be backfilled with `job update --prompt`. Host hooks also record visible user prompts and agent replies in the generated Conversation section, excluding tool calls/results, reasoning, and injected system context. Codex/Claude capture the current transcript turn on Stop; Pi/OMP capture `agent_end`. For ordinary prompts, Stop reads the transcript backward in 64 KiB chunks, stopping at the current turn boundary instead of parsing historical turns. Fallback message IDs may require an earlier turn marker; long UTF-8 lines and incomplete trailing records retain their existing behavior. Known host-context wrappers are excluded even when carried as user messages. Agent output is displayed in one continuous blockquote without timestamp sections. Stable message IDs prevent duplicate capture. See [conversation capture](https://github.com/tenfyzhong/agentix/blob/main/docs/task-board.md#job-conversation-records).

Codex terminal `/plan` questions, option descriptions, and submitted answers are captured as visible conversation text. Accepting a proposed plan with the native `Implement the plan.` action recovers the consecutive preceding Plan-mode turns into the implementation Job, including plan revisions. It replaces only an empty Prompt or that generated acceptance placeholder with the original planning prompt, preserving explicitly authored prompts and review status. Recovery requires the transcript and a proposed plan, stops at the preceding execution turn, and keeps stable message IDs. Upgrade Taskix and this plugin together; ordinary turns retain current-turn-only reads.
