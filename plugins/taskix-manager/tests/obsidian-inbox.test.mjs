...
const header = (entryId = id, status = "COMPLETED", check = "x", revision = 3) =>
    `- [${check}] Title <!-- taskix:entry:${entryId} --> <!-- taskix:entry-state ${status} revision=${revision} --> · [[Job]]`;
const document = (body) => `# Inbox\n\n<!-- taskix:inbox:start project=${project} -->\n${body}\n<!-- taskix:inbox:end -->\n\nKeep notes.\n`;
...
test("Inbox reports stable malformed receipts once and resets after repair", async (t) => {
    ...
    assert.equal(f.notices.length, 0, "no warning before five seconds");
