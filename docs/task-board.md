...
Synchronization upgrades older receipts to this inline format with a revision, preserving entry IDs and authored details. Legacy `[p]` review markers remain readable; synchronization and status repair write the canonical `[r]` marker without changing the entry state or revision. The connected plugin maps `[ ]`, `[/]`, `[r]`, `[x]`, `[-]` to TODO, ACTIVE, PENDING_REVIEW, COMPLETED, CANCELLED through `inbox set-status`. Manual status changes never create Jobs or claim agent leases. Unlinked items can be marked ACTIVE or PENDING_REVIEW and completed directly after reopening if cancelled. Activating a linked item resumes its existing Job. Linked review submission requires ready Tasks; checking a linked item approves its pending verification or explicitly completes an ACTIVE Job whose Tasks are all terminal. Reopening preserves the Job ID and Task history. Returning to TODO requires active leases to be released first. Unsupported edits restore the checkbox and show a notification. See [Inbox checkbox edits](../plugins/taskix-manager/obsidian/README.md#inbox-checkbox-edits).
...
Directory identity is shared by the CLI and IM service through `ProjectDirectory`. A CLI context request discovers the directory once and reuses that identity after Inbox synchronization; each database read still sees fresh ownership and cancellation state. Discovery uses one Git command and never reads the remote. Explicit `project register` reads the remote as registration metadata. There is no process-global directory cache, so entering a new repository or switching worktrees is observed on the next command.

Directory resolution requires an exact canonical root match for non-Git directories; a registered parent does not own its subdirectories. The CLI registers missing non-Git directory Projects during context resolution and implicit Job/Inbox Project selection. Repeated visits reuse the same canonical path, and distinct paths with the same directory name receive distinct Project names. Existing session-owned work and explicit `--project` selections keep their association; existing Jobs are not migrated. Reading context after deleting a non-Git Project can register a new empty Project.
...
Ordinary directory lookup reads one Project through the root index, without reading unrelated Project bodies or canonicalizing their paths. Registration checks the root again and allocates the first available case-insensitive name through the key index inside the write transaction. Concurrent first visits therefore create one Project and one registration event. Deletion removes the lookup row in the same transaction. Database commits and pending-document recovery retain the existing projection protocol.

Historical resolution uses the Task, Job, and Inbox session indexes to find up to two distinct Project IDs; it loads only the matching Project when the association is unique. No-directory/no-session lookups return without querying the database. See [project resolution performance](project-resolution-performance.md) for reproducible measurements and coverage boundaries.
...
`task_board.enable` defaults to `false`; a `config` path alone does not enable the integration. When disabled, Agentix does not load the taskix configuration or start the task board, and IM menus and help omit task-board commands. When `enable = true`, the referenced configuration must exist; its SQLite database is created if missing. Select the same taskix configuration as your task writers to browse their existing work. Run `agentix reload` after adding or changing `[task_board]`; taskix being configured on its own does not enable the IM integration. With `task_board.enable = true`, Telegram registers `/dashboard` in its default command menu at startup, before any attachment. The top-level menu is sorted alphabetically: `/cancel`, `/dashboard`, `/help`, `/rmux`, `/sessions` (omit `/dashboard` when disabled). Contextual commands follow in alphabetical order. `/board`, `/jobs`, `/inboxes`, and `/inbox` appear in the chat menu only after attach; `/tasks` is not added to the menu.

`/dashboard` is the top-level IM dashboard. Each unarchived project has a button that opens its Jobs list without requiring an attached session. Jobs are ordered by modification time descending (Job ID descending breaks ties). The default **All statuses** view includes every unarchived Job; select **ACTIVE**, **PENDING_REVIEW**, **COMPLETED**, or **CANCELLED** to filter. Filtering resets to the first page, and **Previous**/**Next** preserve the selected status.
...
Task-board commands use a consistent layout: overview, each entry description immediately followed by its selection button, navigation, then status filters or task actions. Project Job filters stay at the bottom; task state actions are separate from navigation. Status filtering currently applies only to project Jobs, not session `/jobs`, `/board`, `/tasks`, or Tasks inside Job details. Navigation buttons, including **Previous**/**Next**, use the primary style to distinguish them from neutral entry selection buttons. Primary controls use the channel's primary color and failed/cancelled or destructive controls use red. Telegram uses blue/red button-label indicators and sends each section as an independent message, keeping its controls attached to its description; Feishu and Slack keep the sections in one card. Lists contain at most six entries per page. `/tasks [job-or-project]` also offers task detail buttons and pagination within its existing 50-match limit, with a hint to narrow larger lists. `/board` and `/jobs` include links to each other and Dashboard; project Jobs and Inbox link to each other.
...
Agentix stages completed user message items immediately and refreshes visible messages when the turn ends, even without an IM attachment. Codex and Claude stage the current transcript turn at the first tool call and refresh it on Stop, interruption, and session end when `transcript_path` is supplied; Pi/OMP stage prompts before execution and replies at `agent_end`. Unassigned turns remain SQLite drafts until explicit Job attachment. By default, tool calls, tool results, reasoning, and system/developer messages are excluded. Agentix can include host-exposed reasoning and tool details with `[output] show_reasoning = true` and/or `show_tool_calls = true` in its configuration. These independent switches apply to IM turn output and the associated Job’s Agent output. IM process output uses bold **Reasoning**, **Tool call**, and **Output** labels when process details are present. Labels are separated from their content by a blank line, and adjacent process blocks and the final output are separated by two blank lines. Reasoning start labels are omitted; only actual host-provided reasoning content is shown. IM preserves the first-seen order of Reasoning, Tool call, and Output blocks, including interleaved multiline content. Streaming updates and completed items update only their own block by item ID; final answers retain earlier process blocks and intermediate answers. Cold turn storage preserves the same order. Live turns, Background completion notifications, and history use the same output policy and formatting. Feishu renders typed sections as **👤 You** followed by **🧠 Reasoning**, **🔨 Tool Call**, and **🤖 Agent** blocks in occurrence order. Adjacent process items of the same kind share a panel. The newest process panel is expanded until the next visible block appears, then it collapses; Agent output stays expanded. Older panels can be opened manually. Codex messages marked with the `commentary` phase are process commentary and appear under Reasoning (controlled by `show_reasoning`), while `final_answer` remains the answer; unmarked messages retain their normal answer classification. Streaming commentary stays in its own panel and completion replaces only that item. Other channels retain the Markdown fallback. Tool events are deduplicated by item ID; recording happens at turn completion. Standalone Taskix host hooks continue to capture visible user/assistant messages only. Known leading host-context wrappers, including injected `# AGENTS.md instructions` with `<INSTRUCTIONS>` and `<environment_context>`, are also removed from user-role messages at capture and persistence. Actual requests mentioning AGENTS.md remain intact. Sync applies the same filtering when rendering old stored conversations and prompts. Existing Job prompts are preserved; the first recorded user message fills a missing prompt. Automatic capture applies to new turns after upgrading the host plugin; it does not reconstruct unavailable historical transcripts.

Related discussions are selected when execution begins, for ordinary conversation and Plan mode across all supported hosts. Selection includes earlier questions, alternatives, revisions, and the current implementation request, while excluding unrelated pending turns. Codex `request_user_input` questions, options, submitted answers, and proposed plans remain visible text. New Job creation sets Prompt from the earliest selected request; follow-up preserves the existing Prompt. Attachment preserves original turn order and stable identities, including late final replies and capture from cooperating entrypoints. It does not change review state. Discussion-only sessions create no Obsidian Job note. Draft capture does not require vault access, and unchanged bound captures skip document projection. Use `taskix sync` or a normal document-producing write to repair unrelated pending projections. Deleting a Job also removes its bound source copies while preserving unrelated drafts. Drafts persist across restart and expire after 30 days of session inactivity; bound history remains. No automatic historical Job backfill occurs.

Before creating or following up a Job, the agent invokes the plugin's read-only discussion helper with the current turn and final delivery target. Configured Jev classifies full pending messages; disabled Jev, uncertainty, errors, incomplete context, the 24,000-byte context bound, and the eight-second deadline fall back to the agent reading original turns. The helper checks candidate and target revisions again and returns guarded arguments. `job create` and `job followup` accept repeated `--conversation-turn` plus `--conversation-revision`; ACTIVE Jobs use `conversation attach --job JOB_ID --turn TURN_ID`. Existing Job writes use `--expect-revision`, and new targets can be fingerprint-guarded with `--conversation-target`. Selection and creation/follow-up commit atomically. Changed evidence rejects the write and requires reassessment. `conversation list` provides paginated metadata, `--full` includes original messages, and `conversation show TURN_ID` retrieves one turn. These commands use the actual host `--session`.

A host submits `{ "turn_id": "native-turn-id", "source": "host", "messages": [{ "id": "stable-message-id", "role": "user", "text": "visible text" }] }` through `taskix hook record --session SESSION --file messages.json`. Unbound turns are staged without latest-Job inference; bound turns update their selected Job. The legacy message array and `planning` wrapper remain supported for older producers and retain their automatic eligible-Job selection. Upgrade Taskix and all capture producers together for the explicit discussion workflow. Capture does not claim work or acquire a Task lease.


### Supplementary requests and review policy

By default, context and host hooks expose `previous_job` as a candidate for a new prompt, and the agent decides whether the request supplements that PENDING_REVIEW Job. Optional [host-side Jev routing](../plugins/taskix-manager/README.md#optional-jev-routing) classifies before the Agent runs, includes ACTIVE Jobs with waiting Tasks, and injects only selected context on confident results. Missing configuration retains the default workflow; uncertainty or service errors defer to the Agent. If so, `taskix job followup JOB_ID --prompt "Verbatim supplementary request" --executor agent:HOST --session HOST_SESSION` returns the same Job to ACTIVE and appends the request without replacing its original Prompt. Supply the actual current host identity and session on follow-up to associate subsequent conversation capture with that session. Existing Tasks remain; each Task added for this supplement automatically depends on the snapshot of all old Tasks taken at follow-up. Additional dependencies among new Tasks are configured normally. Cancelled and other unfinished prerequisites do not satisfy the execution gate. Independent requirements and requests after a COMPLETED Job create new Jobs; hooks do not reopen Jobs merely because a prompt arrived.
