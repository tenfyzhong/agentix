...
The IM dashboard aggregates Job and Task counts in SQL without deserializing their bodies. Project and session boards load only associated work, while Job and Task details load the selected records and their parents and leases; these reads omit Plan and Inbox bodies. Sorting and pagination still operate within the selected scope, preserving counts and navigation order. Session associations include stored expired leases, but only unexpired leases mark tasks as current. Markdown dashboard publication uses one batch activity query for all active Projects instead of querying each Project separately.
