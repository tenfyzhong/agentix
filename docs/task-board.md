...
Related discussions are selected when execution begins, for ordinary conversation and Plan mode across all supported hosts. Selection includes earlier questions, alternatives, revisions, and the current implementation request, while excluding unrelated pending turns. Codex `request_user_input` questions, options, submitted answers, and proposed plans remain visible text. New Job creation sets Prompt from the earliest selected request; follow-up preserves the existing Prompt. Attachment preserves original turn order and stable identities, including late final replies and capture from cooperating entrypoints. It does not change review state. Discussion-only sessions create no Obsidian Job note. Drafts persist across restart and expire after 30 days of session inactivity; bound history remains. No automatic historical Job backfill occurs.
