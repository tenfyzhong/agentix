...
`job update`, `task update`, `task depend/undepend`, `plan revise`, `job cancel`, `job archive/unarchive`, `job delete`, and `project delete` provide the remaining mutations. Job deletion cleans up registered document paths and unregistered destinations only when their generated entity identity matches. An unrelated note that blocked creation or renaming is preserved. Project deletion still removes the entire project directory, including attachments. Consult `--help` for each command.
