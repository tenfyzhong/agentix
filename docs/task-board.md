...
Codex terminal plan acceptance (`Implement the plan.`) also recovers the immediately preceding consecutive Plan-mode turns from the transcript, including original planning prompts, revisions, and `<proposed_plan>` replies. Recovery stops at the preceding execution turn and requires a proposed plan; ordinary prompts still read only their current turn. `request_user_input` questions and option descriptions are recorded as assistant text, and submitted choices or free-text answers as user input. Other tool results remain excluded. Recovered messages keep their original turn IDs, so retries are deduplicated and planning history is inserted before an already recorded execution turn. A missing Prompt or the generated `Implement the plan.` placeholder is replaced with the first recovered planning prompt; explicitly authored original prompts and Job review state are preserved. Upgrade both Taskix and the host plugin for this capture format. Existing Jobs are not scanned or backfilled automatically outside a plan-acceptance turn.

A host can submit a JSON array of `{ "id": "stable-message-id", "role": "user" | "assistant", "text": "visible text" }` through `taskix hook record --session SESSION --file messages.json`, optionally with `--job JOB_ID`. The file can alternatively contain `{ "messages": [...], "planning": { "prompt": "original planning request", "implementation_prompt": "Implement the plan." } }` for ordered planning recovery; both prompts must be present in the message batch. Only Jobs associated with that session are eligible; otherwise the most recently worked associated Job is selected. No eligible Job is a no-op. Replaying the same message ID does not duplicate it. A late final response can be recorded after Task completion without changing the Job's review state. The operation does not claim work or acquire a Task lease.
