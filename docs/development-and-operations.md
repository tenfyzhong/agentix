...
Set `multiplexer.kind` to `auto` (default), `rmux`, or `tmux`. At service startup, `auto` probes the native rmux protocol first, then the tmux command interface. Explicit `rmux` requires a successful native probe; explicit `tmux` accepts a successful tmux-compatible response, including rmux. Probes are read-only and never start a daemon. If no applicable probe succeeds, terminal management is disabled and neither terminal command is registered. Start the desired server before Agentix; restart Agentix to detect a newly available server. `multiplexer.working_dir` has been removed and is rejected; remove it from existing configuration files. Directory inference falls back to the current user’s HOME and never to the configuration directory. The selected menu is `/rmux` or `/tmux`. Changing `kind` requires a service restart. Per-agent `rmux_directory` and `multiplexer_directory` have been removed and are rejected.
...
**The proxy endpoint belongs exclusively to Agentix. Do not start `codex app-server` on this address.** On a Unix bind conflict, Agentix probes the existing socket. A socket owned by the current user with no listener is removed before retrying the bind. If the kernel peer PID and process inspection identify a same-user `codex app-server`, Agentix sends that process SIGTERM, waits up to five seconds, and escalates to SIGKILL if the verified listener remains. This interrupts work in the misplaced app-server. Recovery rechecks the socket inode and peer before signalling or unlinking; it never kills another Agentix instance or an unidentified listener. Ordinary files, symlinks, foreign-user sockets, unknown owners, failed inspection, and occupied WS ports still produce an ERROR and nonzero exit. Process inspection uses `ps` with a bounded timeout. Agentix removes only its unchanged socket on graceful shutdown.

When a local upstream is missing or refuses connections, Agentix runs the configured `command` with `app-server --listen ENDPOINT` and waits for readiness. Custom Unix and loopback WS upstreams support automatic startup too; remote WS upstreams must already be running. On graceful exit, Agentix terminates and reaps app-server processes it launched, including backends connected by a startup retry. It sends SIGTERM to the owned process group, allows five seconds for exit, then kills an unresponsive group. Launcher descendants are included, and their unchanged socket is tracked before the readiness handshake so cancellation also cleans it up. The reaper removes the owned socket when app-server exits or is externally killed. SIGKILL of Agentix itself cannot execute cleanup; the next start recovers its abandoned frontend socket. Backends shut down concurrently after channel shutdown. Existing upstreams started outside this Agentix instance remain running. Reload keeps the current upstream. Stopping or restarting Agentix therefore interrupts work in its owned app-server.
