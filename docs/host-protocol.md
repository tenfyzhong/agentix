...
Native bridging requires a Unix endpoint on macOS/Linux. Loopback TCP control remains available for ordinary local clients; native registration is unavailable there. Do not expose a control endpoint through an untrusted proxy.
