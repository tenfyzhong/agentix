...
**The default rmux delivery avoids this dependency.** It submits ordinary terminal input without changing Claude's authentication or provider configuration. The plugin still connects to Agentix's existing Unix socket for requests, events, history, and delivery receipts.
