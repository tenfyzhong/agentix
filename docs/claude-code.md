...
Keep the original terminal running. Use `/sessions claude` in IM to select and attach the session. Leave Claude idle in insert mode, then send an ordinary message in IM. Before pasting, Agentix reads the existing terminal draft (including multiline text) and shows it in IM with **Clear and send** and **Cancel sending**. Only confirmation clears the matching draft with Ctrl+C; cancellation preserves it. A changed draft is shown again for confirmation. An already empty input box is left alone. The adapter then pastes and submits the IM text through the original terminal. A matching `UserPromptSubmit` hook confirms receipt; the `Stop` hook reports the answer and completes the turn. Model cooperation with an acknowledgement tool is not required for terminal delivery.
...
- The adapter checks the pane's original process ancestry, active command, cursor, and surrounding prompt borders, then verifies the cleared input row. Existing drafts require explicit IM confirmation before clearing and submission. A dialog, copy mode, non-insert Vim mode, unrecognized layout, or unsuccessful clearing prevents submission. An already active turn is rejected.
...
IM `/new` requires terminal delivery with the original Claude process in rmux/tmux. It waits for active work to stop, validates an empty Claude input row, and submits `/clear`. Starting `/clear` locally also transfers the IM binding using the `SessionEnd` clear reason and `SessionStart` clear source. Parent process identity and the previous session ID distinguish replacement from resume or another Claude process. Messages sent during the gap are retained by Agentix until the replacement is attached, or paused after 120 seconds. This does not enable arbitrary slash commands through prompt delivery.
