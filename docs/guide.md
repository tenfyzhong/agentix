...
Install and start rmux or tmux before Agentix to create new agent sessions from Telegram, Feishu, or Slack. The default `multiplexer.kind = "auto"` selects a responding rmux service first, then tmux. Explicit `rmux` verifies the native protocol; explicit `tmux` accepts a responding compatible interface. Only a detected backend exposes its `/rmux` or `/tmux` command; neither command appears when no backend is available. Restart Agentix to refresh detection. Creating a session, window, or split first previews its directory. Confirm with `Create` to open an empty shell pane, then choose a configured agent to launch in it. Existing idle panes also offer `Run agent`. The directory is inferred from the attached agent or target terminal, falling back to the current user’s HOME. `Choose directory` supports browsing and entering a path for this creation only; no task board is required. A multiplexer is optional for connecting to existing Codex sessions.
...
Service managers must allow time for the entire shutdown sequence, including work draining, state persistence, IM effects, and child reaping. The Homebrew formula uses `stop_timeout 30` (launchd `ExitTimeOut` / systemd `TimeoutStopSec`). A five-second supervisor deadline can kill Agentix before its five-second Codex termination grace finishes, leaving the separate upstream process group alive. After state preparation, backend shutdown runs concurrently with channel teardown so a slow channel does not postpone child cleanup. Formula changes take effect when the service definition is regenerated; an already loaded launchd job keeps its old timeout until restarted. A surviving external upstream is still preserved, not killed merely because Agentix restarted.
...
While `agentix serve` is running, Agentix checks running Codex sessions for completed turns every ten seconds using read-only history queries, including sessions that have never been attached or were detached from IM. Background monitoring does not resume sessions or acquire their writer locks. Completion events finish local turn state before optional content reads and IM delivery run in a separate bounded queue. Cached prompt and answer content avoids a history read. Otherwise, a slow lookup publishes a loading notice and later updates the same logical card. Updates to one card are serialized and obsolete pending states are skipped. An uncertain update retires that message and sends the latest state as a replacement card, preventing a late old update from overwriting the replacement. If the replacement send is also uncertain, automatic retries stop to avoid duplicate messages. Local queue/cooldown waits have a separate 60-second operation budget; a dispatched request has a five-second response budget. Cancelling an unsent request or receiving a definite rejection preserves retryability. Disabling buttons preserves pending answer content. Content reads retry once after an error or timeout, with a five-second limit per complete lookup and a 100 ms retry delay. If both attempts fail, the notice retains available cached output and its Attach button, and reports missing content; this does not mean the agent turn failed. At most four notices run concurrently and 64 are retained in total; excess optional notices are skipped with a diagnostic warning so event ingestion continues. This queue is best effort, not a durable delivery outbox. See [Background completions](background-completions.md) for cancellation, limits, and validation. New completions include the completed turn's prompt and response, a Background label, and an Attach button. Feishu uses a purple header and a grey quote area; Telegram uses a ⚫ Background marker and blockquotes. Notifications go to authenticated IM conversations known to the service. Codex subagent sessions do not generate standalone completion notices; parent sessions and existing attached or draining turn cards continue updating normally. Send the bot `/help` once to register a conversation for these notifications; attaching a session is optional.
...
Only the current attached session's writable active turn message has Stop; switching sessions, moving the attachment to another conversation, detaching, or finishing the turn removes it from the previous message. Copies shown by `/history` never include Stop. `/last` sends the latest attached turn as a new card and moves its live updates and Stop action to that card, preserving the original reasoning, tool calls, and output layout.

Starting `agentix serve` restores saved conversations and sends their online status without posting command menu cards to Feishu or Slack. Telegram's native command menu is still synchronized. Send `/help` to view the available commands. Help lists first-level commands first, then a blank line and second-level commands for the attached session. Each group is sorted alphabetically; unavailable commands are omitted.
...
The default is `true`. Disabling notifications also stops automatic background turn polling and full-content reads. When no sessions need exit/resume monitoring, automatic session discovery stops too. Existing attached or draining turn cards still complete, using replacement cards when an update is uncertain; attached-session exit/resume monitoring remains active. Both completion deduplication caches keep only the latest completed turn per session, with recipient tracking for that turn, so records do not accumulate for every completed turn.
...
| Command | View |
| --- | --- |
| `/dashboard` | Project dashboard; click a project to browse jobs by latest update and filter by status |
