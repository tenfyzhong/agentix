...
Nonempty registries also reclaim peak container capacity. After an ordinary background sweep, a registry with at least 1,024 slots in either container and at most one-quarter as many entries starts incremental compaction. Starting compaction moves the old containers without scanning them. Each subsequent tick visits at most 256 old keys, reclaiming idle healthy entries and moving surviving entries into fresh containers. These ticks replace the ordinary background sweep rather than adding another sweep budget. All physical aliases participate in the queue. Lookups consult both tables during migration; new keys enter the fresh table, and existing keys keep their logical card and revision state. Hot lookups never start or advance compaction. The old allocations are released when migration finishes, including when retained safety mappings prevent the registry from becoming empty.

The threshold avoids rebuilding small or densely populated tables. Compaction temporarily owns both old and new container allocations, but does not copy message strings or card state. Candidate counts are bounded; hash-table growth and allocator work still prevent a hard wall-clock bound per tick. Releasing container allocations also does not guarantee an immediate process RSS reduction. Safety mappings themselves remain proportional to uncertain writes and are not expired by compaction.
...
## Independent history and recipient progress

History lookup and notification delivery are polled concurrently inside the same owned completion task. Loading-card queue waits and provider cooldowns no longer suspend the five-second history deadline or its retry. The final rendered view is shared once ready.

Each job runs at most four recipient flows concurrently, including cached and immediate-history delivery. With four active jobs, this permits at most 16 concurrent recipient flows; existing adapter pacing still applies. Each recipient finishes its own loading write before its final write. A fast recipient can finalize while another loading delivery remains blocked. Recipients admitted after history completes skip the loading view. If every recipient slot is occupied by slow sends, additional recipients still wait for a slot; this is bounded concurrency, not unlimited isolation.
