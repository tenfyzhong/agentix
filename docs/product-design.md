...
Telegram converts agent Markdown to MarkdownV2 in bounded UTF-8 text messages, registers a native command menu, and uses two-column inline keyboards. Feishu uses shared Card JSON 2.0 documents with a status-colored header, title/subtitle, Markdown body, and callback buttons. Attachment silently synchronizes native menus without posting command cards to Feishu or Slack; users can send `/help` to view available commands. Feishu uses an editable interactive command card for other menu updates because it has no runtime API for per-conversation native bot menus. Slack uses bounded Block Kit sections and buttons, with an editable command message and app-wide slash commands synchronized through Slack CLI during adapter initialization. All presentations are generated from the same channel-neutral `OutboundView` and `CommandMenu` models.
