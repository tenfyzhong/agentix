...
`agentix reload` uses the same local control endpoint and a server-owned configuration path. A supervisor keeps the listener and native bridge hub alive while preparing a replacement runtime. Matching backend adapters are reused; configuration validation and preparation failures leave the active generation running. The old engine and channel workers drain before replacement binding restoration; a restoration failure resumes the previous engine. This transition skips final shutdown detachment and offline notices. Listener, storage, logging, and existing Codex transport changes are rejected with a restart requirement. See [configuration reload](development-and-operations.md#reloading-configuration) for the supported settings and request behavior.
