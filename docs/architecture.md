...
See [Session lifecycle and message flows](session-lifecycle.md) for Codex, Pi, OMP, and Claude discovery, attachment, streaming, replacement, exit, and recovery diagrams.
...
Session-switch recovery reserves its current backend exclusively because it can attach a persisted replacement not yet present in the routing snapshot. This orders replacement events before or after the handoff while other known backends remain independent. Without a current binding, recovery conservatively reserves all registered backends. The reservation lasts for the recovery operation, including any attachment or delivery I/O.
...
Bridge transport errors distinguish a failure before dispatch from a lost response after a mutating request was fully written. The latter returns `AgentError::Uncertain`; Engine durably fences the inbound event against automatic replay because the original process may already have executed it. Read-only RPCs and failures before dispatch remain retryable. Completed command notices are best effort and do not suppress inline plan prompts. Stop actions are revoked before their message projection is updated; a failed message edit cannot block replacement binding. If a command creates a replacement but its local binding cannot be persisted, the inbound event is fenced and the error identifies the created session for explicit reattachment. If an uncertain command and its error notice both fail, the original uncertain outcome takes precedence over the notification error. A new user request has a new event identity; users should inspect session state before resubmitting an operation with an uncertain outcome.

Codex RPCs use a 30-second deadline covering writer admission, WebSocket send and response delivery. A request owns its pending response slot through an RAII guard, so completion, timeout and cancellation all release it synchronously. Once a mutating send begins, transport failure has an uncertain outcome and uses the same inbound fencing contract. Reads and explicitly idempotent resume/unsubscribe operations retain their retry behavior. Once `turn/start` succeeds, a subsequent subscription recovery failure preserves that success and leaves the pending subscription for background recovery.
...
Feishu paginates `OutboundView` inside its adapter. A page budget includes the serialized card, JSON string escaping, and an envelope reserve; section count is also bounded. Short views retain their original layout, while long views use ordered continuation cards with all actions on the first physical message, which remains the logical `MessageRef`. A whole group send/update/disable operation holds one conversation FIFO position. Each physical request retains token refresh, rate-limit handling, and delivery-scope checks; sends use distinct UUIDs stable across retries. Updates remember each successful page before proceeding, and retries skip unchanged pages. Surplus pages are recalled after a shrinking update. An initial send rejected partway through recalls confirmed pages before returning; failed cleanup reports transport uncertainty. Groups are not atomic remote transactions, and cancellation or unknown delivery outcomes can leave partial output.

Card-group IDs and rendered pages are in-memory adapter state shared across clones, not durable routing state. Replacing an adapter or restarting the process loses continuation ownership and can leave old continuation messages when a turn is restored. Single-card views without active actions are not retained. `render_cards` exposes capacity-based rendering; `render_card` rejects views requiring more than one card rather than returning truncated content.
...
Explicit IM attachment first attempts `thread/resume`. A newly created session without a rollout remains provisionally attached while subscription recovery retries. Once recovery succeeds, the adapter restores the latest turn snapshot, including its input, to cover events emitted before the subscription existed. If the original client acquires the writer in the meantime, recovery switches to read-only observation from an empty cursor so the first turn is still delivered. An active-writer rejection falls back to a read-only attachment with full history snapshots every ten seconds, independently of background-notification settings. Observed sessions survive transport reconnects without another resume attempt and detach without an unsubscribe request. Changed snapshots produce item and turn events for the existing IM renderer; these connections expose no write controls or Stop actions. The adapter rejects session mutations locally. Other attachment or initial-history failures show an IM error and a fresh retry action before changing the durable binding. Session discovery derives activity from the latest saved turn when the app-server reports `notLoaded`; this is a history-based estimate, not live status from the owning process.
...
A terminal event without a live or draining route is a background completion. The Engine snapshots the exact turn, terminal state and authenticated recipients, admits bounded optional work, then archives the turn and completes draining cleanup before releasing a delivery permit. Disabled transports are skipped. Current-session completions stay on their live card. Standalone notices suppress subagents and failed source queries through a bounded `AgentAdapter::is_subagent` check; draining cards bypass that check.

`BackgroundCompletions` owns at most four active jobs and 64 total jobs, deduplicates in-flight keys and remembers 256 recent completions. Complete cached prompt/answer content avoids history lookup; other jobs read history with a five-second deadline and one retry. Slow reads show a loading card whose final update retains available content and explains missing history. History and delivery progress concurrently within each owned job; at most four recipient flows run per job (16 across four jobs), each preserving loading/final order. Recipients that start after history completes skip loading. Adapter pacing remains shared. Compatible Engine reloads reuse the coordinator; incompatible reloads and shutdown cancel workers. Optional queries and IM delivery never hold an Engine dispatch lane.

`CardWrites` serializes edits and control disabling per logical card across all Engine paths. Revisions discard obsolete pending states; a separate control barrier preserves pending answer content when buttons are consumed. `DeliveryAttempt` distinguishes local admission from remote dispatch, with a 60-second operation budget and five seconds per dispatched request. Definite failures remain retryable. Unknown edit outcomes retire the physical message and use a replacement, sharing the logical writer across both IDs so a late old request cannot overwrite the replacement. An uncertain replacement send stops automatic retries. These mappings survive Engine reloads, but not a full process restart.

Loading/final cards reuse an owner-, conversation-, connection- and binding-scoped Attach token. Failed edits do not revoke the visible token; consumed tokens are not recreated, and an already attached target shows disabled `Attached`. Structured sections and fallback text both contain loading/missing-content explanations. Session titles use a shared bounded metadata reader with a 50 ms initial wait and a native-ID fallback. `ViewStatus::Background` supplies the purple Feishu header and grey container, while Telegram uses a Background marker and standard blockquotes. Completion, failure and interruption remain in the subtitle.

`notifications.background_turns` defaults to true and controls standalone notices and Codex automatic discovery. Disabled notices skip optional metadata/history reads; attached and draining lifecycle handling remains enabled. See [background completions](background-completions.md) for the limits, ownership model, failure semantics and regression coverage.
...
If a local upstream is unavailable, Agentix starts the configured executable with `app-server --listen ENDPOINT`, after reading the effective user's login environment. The child starts in a separate process group with detached standard streams. Startup failure or cancellation terminates the newly launched child. After readiness, a reaper retains the child handle until natural exit or shutdown. Graceful service exit invokes backend shutdown after channels and control handlers finish; deferred backends cancel and join connection attempts before closing their connected adapter. Owned app-servers receive SIGTERM and have five seconds to exit before forced termination and reaping. Backend shutdowns run concurrently. Concurrent or cancelled waiters share the same child shutdown, and final-owner drop requests cleanup with kill-on-drop as a runtime-teardown fallback. Only an unchanged upstream socket whose peer PID matches the launched child is removed; a listener that wins a startup race is not claimed. Existing upstreams are adopted without process or socket ownership and remain running. Reload keeps the current backend; service stop/restart interrupts active work in owned upstreams. SIGKILL and process crashes cannot guarantee graceful cleanup. The proxy endpoint belongs exclusively to Agentix; users must not start app-server on it. Unix or WS bind failures become `ProxyBindError`: `serve` logs an ERROR and exits nonzero before starting the upstream or IM channels, without entering backend retries. Existing Unix paths, including stale sockets and regular files, are preserved. Graceful shutdown removes only the proxy socket inode created by this instance.
...
The proxy records connection IDs, operating-system peer PIDs where available, client names, and successful `thread/start`, `thread/resume`, and `thread/fork` results. Server broadcasts and failed responses never create ownership. Successful unsubscribe removes that connection's association; EOF, disconnect, or proxy shutdown removes all its associations. Unsubscribe alone does not announce a native `/new`: Ctrl-D uses the same RPC. A non-ephemeral replacement start confirms the switch. The lifecycle monitor retains the old attachment candidate for up to two minutes only while its connection remains open, without listing the old session as active or sending a switch notice. Closing the connection immediately releases the candidate and wakes exit processing, even if the client process is still alive. Exit cleanup preserves completed, failed, and interrupted turn results and their content; only unfinished turns become interrupted. Replacement correlation does not cross a closed connection. Confirmed replacement events on the same connection precede departure cleanup. Multiple clients may own the same session independently. Unix uses peer credentials; WS and stdio leave PID unknown. PID is diagnostic metadata, never the liveness or ownership key. No working-directory heuristic is used by the proxy runtime.
...
The public client clones jointly own a connection-layer task guard. Dropping the last clone aborts the internal reader/reconnect loop, lifecycle monitor, and attached-content monitor, releasing their upstream socket and pending-request state when cancellation is processed. Background tasks never own this guard or the proxy runtime, avoiding a self-retaining ownership cycle. Dropping one clone leaves the remaining clones operational. Proxy runtime drop cancels the listener and its accepted connections, removes registrations, and unlinks only its own Unix socket inode. The detached, ready shared app-server and its upstream socket survive this cleanup.
...
Foreground session listing reads registered IDs and fetches their metadata from the upstream, including new threads without a rollout file. In proxy registry mode, lifecycle changes, attached-content refreshes and background history reads are notification-driven: there is no 250-millisecond content timer or ten-second reconciliation timer. The proxy extracts read-only routing hints from item, turn and thread-status notifications and refreshes only the owning session. Routing skips string payloads without decoding them and stops at the thread ID; ownership changes and interactive requests still require complete JSON validation. Content notifications do not wake the lifecycle monitor. Attach, detach, configuration changes and upstream reconnect also wake processing. Each attached session has at most one recovery or content read in flight. Content reads use a 100-millisecond minimum spacing through one-shot waits only when dirty; the latest version is captured when the read starts, merging notifications received while waiting. Initial recovery is immediate, and changes during a read trigger a follow-up. Background history runs independently, reads only newly registered/departed sessions or sessions with completion notifications, and excludes attached sessions before querying. It also performs initial reconciliation when enabled. The legacy direct API retains its original ten-second polling interval. A native switch candidate has a one-shot expiry deadline. Optional session-title lookups have a one-second budget and fall back to the session ID. Registered attach attempts defer upstream work after one second and recover on notifications; provisional history is empty until recovery completes. Agentix's direct upstream subscriptions do not make a terminal session appear active. The legacy direct client API retains process/loaded-thread discovery and polling because it has no proxy notification source; `serve` uses the proxy registry. Raw `agentix/clients` requests are answered locally with current registrations.

Session exit invalidates local bindings and actions and sends its notice before updating old cards. Card finalization shares a one-second budget (also applied during native `/new` exit cleanup); slow IM edits cannot hold subsequent lifecycle events indefinitely. Finished turns retain their terminal status, while running or unknown turns become interrupted.
...
The workspace runtime depends on `agentix-multiplexer`, which owns the `MultiplexerDriver` contract and common workspace validation, name allocation, session matching and UI inventory conversion. The application composition root selects `agentix-rmux` (typed SDK) or `agentix-tmux` (bounded CLI subprocesses); Codex, Bridge and core do not import concrete drivers. Global `multiplexer.kind` applies to all agents. The application resolves HOME once; core infers cwd from the attached agent or target pane, with HOME as the fallback. Shared runtime methods resolve paths and list directories without consulting taskix or project registration. Creation drafts are scoped to conversation, owner, selected backend, connection generation and binding epoch, and are consumed only after directory confirmation. Reusing an idle shell pane clears its draft before a structured process launch. Each conversation action captures its selected agent, while terminal identity includes the multiplexer kind to prevent equal pane IDs from colliding. Native attachment requires a live registration in the created pane.

All terminal CLI operations use a shared command runner: Codex native `/new` and composer inspection, tmux inventory and workspace operations, and process queries. On Unix, the runner obtains PATH from the user's login shell before starting any command, including explicitly configured executable paths (`AGENTIX_LOGIN_SHELL` overrides the account shell). It never attempts the service PATH first or retries execution. Invalid, failed, or timed-out login lookup rejects the operation without starting the target command. Login lookup is bounded to three seconds within the five-second operation deadline. Arguments remain structured, other inherited environment values and PID/pane/draft safety checks are preserved, and startup errors identify the requested executable.
...
- Failed IM delivery: retain the event ID in a retryable state and execute it again on redelivery. Preparation failures, including conversation-owner persistence, also release the claim before any backend execution.
...
- Turn IM send/edit failure: preserve the turn buffer so a later final update still has full content. Backend completion clears active-turn routing before the final IM edit, so a failed edit cannot steer subsequent input into the finished turn.
- Accepted queued prompt: confirmation-card failure is logged without making the input retryable; redelivery cannot enqueue the accepted prompt again. `/queue` can inspect the backend queue.
...
Codex asynchronous questions carried by `agentMessage.questions` on `item/completed` are projected as separate user-input interactions while preserving the ordinary item-completion event. Their internal object identity separates them from real RPC request IDs. IM answers are sent as ordinary follow-up text through `turn/start`, matching the CLI editor semantics, rather than as JSON-RPC responses. This explicit answer path is allowed for an observed CLI session without lifting the general read-only request guard. The proxy deduplicates observed questions and records successful IM answers; the CLI's local question-editor state is not remotely cleared.

## Taskix discussion storage and attachment

Host capture and Job ownership are separate operations. Capture stores visible messages under stable session/turn/message identities in SQLite; it cannot select the latest Job. The read-only discussion helper proposes a selection using configured Jev, or defers to the agent when context is incomplete, uncertain, oversized, or unavailable. Job creation, follow-up, and explicit attachment validate the selection and target guards inside the same transaction as the lifecycle write. A conflicting selection rolls back the entire write.

Unbound capture does not deserialize a previous Job or acquire an Obsidian output lock. Identical bound capture also skips Job loading and projection. These paths do not repair unrelated pending projections; ordinary document-producing writes or `taskix sync` perform that recovery. Changed bound capture reads only the current turn body and one later-turn message identity as an ordering anchor. It does not replay all bound source bodies. Indexed metadata queries serve the pending-turn list without deserializing message bodies.
