...
`serve` owns a registry of independently connected backend adapters shared by its IM channel and local CLI clients. Config selects backends through `[agent.codex]`, `[agent.pi]`, `[agent.omp]`, and `[agent.claude]` without `kind`. The decoding boundary normalizes these tables into runtime `AgentConfig` values. Legacy `[agent]` and `[[agents]]` inputs remain compatible but cannot mix with named tables. Engine, action routing, and persisted bindings use backend-qualified session IDs; adapters and taskix retain native IDs. An unavailable backend is retried without stopping the other backends. It exposes a small newline-delimited JSON protocol over `unix://~/.local/share/agentix/control.sock` by default on macOS/Linux or `tcp://127.0.0.1:32198` on Windows. Ordinary CLI connections carry one request and one response; native registrations upgrade to persistent bidirectional streams. CLI requests list sessions, execute shared session operations, pass a raw RPC through the existing Codex connection, or create a temporary owner claim for the selected IM channel. This keeps client diagnostics consistent with the live service and avoids a second direct Codex app-server connection.
