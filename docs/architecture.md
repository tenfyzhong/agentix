...
The Codex runtime polls running-session discovery every two seconds, following session-list pagination and subscribing to sessions independently of IM bindings. This monitor runs even when there are no attached sessions, on both managed and custom Unix sockets. Failed subscriptions are retried on later polls; an explicit unsubscribe releases the transport subscription immediately, and a still-running session is rediscovered for background observation. The mock app-server tracks subscriptions per connection and suppresses events for unsubscribed threads.
