...
With [task boards configured](docs/guide.md#im-task-boards), use `/dashboard`, `/board`, and `/jobs` to browse work. Use `/inboxes` to view the current project's human queue and `/inbox <content>` to append a requirement; agents pick it up after existing Jobs finish. See [Project inbox](docs/task-board.md#project-inbox) for document submission and cancellation.
