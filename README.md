...
- Standalone `taskcli`: SQLite jobs with Mermaid task dependency graphs showing seven task statuses in a light palette shared with TaskNotes and clickable note links, concurrent task claims, task notes tagged `task` and `agent/task` with prerequisite and revision metadata, audit events, and generated TaskNotes boards in Obsidian or Markdown directories
