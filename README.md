...
- Standalone `taskcli`: SQLite jobs, concurrent task claims, task notes with revision metadata, audit events, and generated TaskNotes boards in Obsidian or Markdown directories
...
For a step-by-step guide to TaskNotes, custom statuses, and the optional CSS snippet, see [Obsidian setup](plugins/agent-task-manager/obsidian/README.md).

Both document formats generate `Projects/<project-key>/Board.md` with an embedded TaskNotes Kanban Base. The Dashboard lists only unarchived projects and links to their metadata and Board; it does not list Job details. Project paths use readable names with suffixes only for collisions. Job and Task note filenames use `YYMMDD-seq-name.md`, for example `260905-0001-Implement login.md`. Each project has separate daily Job and Task sequences, padded to at least four digits and based on the UTC creation date; renaming, Plan updates, and archival preserve the prefix. Unarchived Jobs live directly in `Jobs/`; archived Jobs live in `Jobs/Archived/` without year/month subdirectories. Every document has YAML properties and a type tag. Each Task is a note identified by `agent/task` in frontmatter; Job task sections directly link these notes. Jobs use `agent/job` while unarchived and only `agent/archived/job` while archived; restoring a Job switches the tag back. Job properties omit display names, titles, document paths, and embedded Task lists; project metadata and sync status live in `meta.md`. Every Task has one file under `Tasks/`, including Tasks without a Plan. TaskNotes reads its status, dates, project links, and revision from frontmatter. Its body contains the agent-authored plan, with no required section structure. Task timestamp properties use the computer’s local time zone and include an explicit UTC offset. `plan create/revise` publishes that body in place. Task references in Job documents display the Plan filename without `.md`, including its date and sequence prefix. Completed tasks stay in DONE until their Job is archived. Set `AGENT_TASK_LANG=zh-CN` in the agent host environment for Chinese task decomposition and authored names, goals, and Plans. Language belongs to the skill; taskcli stores supplied text unchanged and uses fixed English template labels. `taskcli project archive PROJECT_ID` hides a closed project; `project unarchive` restores it. `taskcli job delete JOB_ID` removes a Job, its Tasks, and their Task notes. `taskcli project delete PROJECT_ID` removes all project work and the entire `Projects/<project>/` document directory, including attachments. Deletion requires released Task leases and no dependencies from surviving Jobs; failed file cleanup can be retried with `taskcli sync`. Obsidian mode uses `[[wikilinks]]`; Markdown mode uses relative `[label](path.md)` links and does not require a vault directory. To render the plugin views, open the files in Obsidian Reading view or Live Preview with TaskNotes and Bases enabled. Other editors can read Job links and individual Task notes; Bases views remain code blocks.

Task state changes still go through CLI commands or Agentix IM actions. Plugin edits are never imported into SQLite and the next projection overwrites them; there is no file watcher. Native TaskNotes controls remain editable. After upgrading all writers, run `taskcli sync` to migrate existing names and Plan files and refresh Board. Configure TaskNotes to identify the `agent/task` tag and recognize the seven status values described in the guide. Schema 7 moves existing Plans into Tasks while preserving authored content. See the [task board guide](docs/task-board.md) for plugin setup, ownership, and read-only limitations.

TaskNotes is the only community plugin required for these views, alongside Obsidian's built-in Bases core plugin. The vault is not a complete SQLite export, and taskcli has no vault-to-database restore command. Back up the database, documents, and configuration together; see [data coverage and recovery](docs/task-board.md#data-coverage-and-recovery).
